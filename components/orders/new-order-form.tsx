...
import { ArrowLeft, Check, ChevronDown, Minus, Plus, Search, X } from 'lucide-react'
...
function CustomerSelector({
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--color-muted)' }}>
            ...
            <span
              ...
              style={{
                color:
                  selected.customer_type === 'a_la_carte'
                    ? 'var(--color-ember)'
                    : 'var(--color-green)',
              }}
            >
  ...
  return (
    <div>
      ...
      {filtered.length > 0 ? (
        <div
          className="mt-1.5 rounded-[11px] overflow-hidden"
  ...
  return (
    <div className="flex items-center gap-2 flex-shrink-0">
      <button
        ...
        style={{
          background: qty > 0 ? 'var(--color-cream)' : 'transparent',
          border: '1px solid var(--color-border)',
          color: qty > 0 ? 'var(--color-ink)' : 'var(--color-border)',
        }}
      >
        <Minus size={12} />
      </button>
      <span
        className="w-6 text-center text-sm font-bold num"
        style={{ color: qty > 0 ? 'var(--color-ink)' : 'var(--color-border)' }}
      >
        {qty}
      </span>
      <button
  ...
  return (
    <div className="py-8 flex flex-col items-center text-center gap-4">
      ...
      <div>
        ...
        <p className="text-sm mt-1 num font-semibold" style={{ color: 'var(--color-saffron)' }}>
          {orderNumber}
        </p>
        <p className="text-sm mt-0.5" style={{ color: 'var(--color-muted)' }}>
          {customerName} · {currency} {total}
        </p>
      </div>
      <div className="flex flex-col sm:flex-row gap-2 w-full max-w-xs mt-2">
        <Button variant="primary" onClick={onNewOrder} className="flex-1">
          New Order
        </Button>
...
export function NewOrderForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-1.5">
              {PERIODS.map((p) => (
                <button
                  ...
                  style={{
                    background:
                      mealPeriod === p.value ? 'var(--color-ink)' : 'var(--color-cream)',
                    color: mealPeriod === p.value ? 'var(--color-cream)' : 'var(--color-muted)',
                    border: '1px solid',
                    borderColor:
                      mealPeriod === p.value ? 'var(--color-ink)' : 'var(--color-border)',
                  }}
                >
                  {p.label}
                </button>
      ...
      >
        ...
        {periodItems.length === 0 ? (
          ...
          <p
            ...
            style={{ color: 'var(--color-muted)' }}
          >
            No available items for this meal period.{' '}
            <Link href="/menu" className="font-semibold underline" style={{ color: 'var(--color-saffron)' }}>
              Add items in Menu
            </Link>
          </p>
          )
            ...
            {periodItems.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderTop: idx > 0 ? '1px solid var(--color-border)' : undefined,
                    background: isSelected ? 'var(--color-cream)' : undefined,
                  }}
                >
                  <div className="flex-1 min-w-0">
                    <p
                    ...
                    {isSelected && (
                      <p
                        className="text-[11px] font-semibold num mt-0.5"
                  ...
                  <p
                    className="num text-sm font-semibold flex-shrink-0 mr-1"
                    style={{ color: 'var(--color-muted)' }}
                  >
      ...
      {/* Cart summary — only shown once items are selected */}
      {cartItems.length > 0 && (
        <div
        ...
        >
          ...
          {cartItems.map((item) => (
            <div key={item.id} className="flex justify-between text-sm">
              <span style={{ color: 'var(--color-ink)' }}>
                {item.name}
                <span className="ml-1 font-semibold num" style={{ color: 'var(--color-muted)' }}>
                  ×{item.qty}
                </span>
              </span>
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2">
              <label className="text-sm flex-shrink-0" style={{ color: 'var(--color-muted)' }}>
                Discount ({currency})
              </label>
              <input
                type="number"
            ...
            >
              ...
              <span
                className="num font-extrabold text-[17px]"
                style={{ color: 'var(--color-ink)' }}
              >
      ...
      >
        ...
        <textarea
          ...
          rows={2}
          placeholder="Spice level, packaging notes…"
